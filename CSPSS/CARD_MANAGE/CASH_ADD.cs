...
{
    public partial class CASH_ADD : Form
    {
        ...
        private void CASH_ADD_Load(object sender, EventArgs e)
        {
        ...
        private void bind()
        {
            ...
            dt = bc.getdt("SELECT * FROM CASH_ADD");
            dataGridView1.DataSource = bc.getdt(cCASH_ADD.sql);
            dgvStateControl();
            textBox2.Focus();
        ...
        protected void save()
        {
            ...
            cCASH_ADD.EMID = LOGIN.EMID;
            cCASH_ADD.CDKEY = CDKEY;
            cCASH_ADD.CDID = textBox1.Text;
            cCASH_ADD.CSID = bc.getOnlyString("SELECT CSID FROM CASH WHERE CARDID='"+textBox2 .Text +"'");
            cCASH_ADD.CARDID = textBox2.Text;
            cCASH_ADD.HANDLER_ID = comboBox1.Text;
            cCASH_ADD.BILL_DATE = dateTimePicker1.Value.ToString("yyyy/MM/dd").Replace ("-","/");
            cCASH_ADD.CASH = textBox3.Text;
            cCASH_ADD.REMARK = "";
            cCASH_ADD.save();
            //hint.Text = cCASH_ADD.ErrowInfo;
     
            ADD_OR_UPDATE = cCASH_ADD.ADD_OR_UPDATE;
        ...
        private void dgvStateControl()
        {
            ...
            dataGridView1.Columns["冲值单号"].Width = 100;
            ...
            dataGridView1.Columns["冲值金额"].Width = 80;
            
            dataGridView1.Columns["制单人"].Width = 80;
            dataGridView1.Columns["制单日期"].Width = 120;
  
            dataGridView1.Columns["冲值金额"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
        ...
        private void save1()
        {
            ...
            else
            {
                ...
                if (cCASH_ADD.IFExecution_SUCCESS && cCASH_ADD.ADD_OR_UPDATE == "ADD")
                {
                    ClearText();
                    bind();
                }
                else if (cCASH_ADD.IFExecution_SUCCESS && cCASH_ADD.ADD_OR_UPDATE == "UPDATE")
                {
                    bind();
        ...
        private void btnDel_Click(object sender, EventArgs e)
        {
            ...
            {
                if (bc.JuageDeleteCASH_MoreThanStorageCASH(textBox1.Text,bc.getOnlyString ("SELECT CSID FROM CASH WHERE CARDID='"+textBox2 .Text +"'")))
                {
                    hint.Text = bc.ErrowInfo;
                }
                else
                {
                    basec.getcoms("DELETE CASH_ADD WHERE CDID='" + textBox1.Text + "'");
