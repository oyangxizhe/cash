...
EMPLOYEEINFO A ";
        ...
        private void dgvEmployeeInfo_DoubleClick(object sender, EventArgs e)
        {
            if (dataGridView1.Enabled == true)
            {
                ...
                if (select == 0)
                {
                    CSPSS.BASE_INFO.CASH.EMID = inputarry[2];
                    CSPSS.BASE_INFO.CASH.ENAME = inputarry[0];
                    //CSPSS.BASE_INFO.CASH.DEPART = inputarry[1];
                    CSPSS.BASE_INFO.CASH.IF_DOUBLE_CLICK = true;
                }
                if (select == 1)
                {
                    CSPSS.CARD_MANAGE .CASH_ADD .EMID = inputarry[2];
                    CSPSS.CARD_MANAGE.CASH_ADD.ENAME = inputarry[0];
                    //CSPSS.BASE_INFO.CASH.DEPART = inputarry[1];
                    CSPSS.CARD_MANAGE.CASH_ADD.IF_DOUBLE_CLICK = true;
                }
                if (select == 2)
                {
                    CSPSS.CARD_MANAGE.CASH_CONSUME.EMID = inputarry[2];
                    CSPSS.CARD_MANAGE.CASH_CONSUME.ENAME = inputarry[0];
                    //CSPSS.BASE_INFO.CASH.DEPART = inputarry[1];
                    CSPSS.CARD_MANAGE.CASH_CONSUME.IF_DOUBLE_CLICK = true;
                }
                if (select == 3)
                {
